...
**Status:** Superseded — decisão de 2026-08-27 revertida em 2026-08-28 após leitura do código nativo da lib escolhida revelar que ela não suporta streaming (ver emenda v1.2 em §8); implementação atual usa `react-native-tcp-socket` (Alternativa 2.2)
**Data:** 2026-07-18
...
- **2026-08-28 (v1.2) — REVERTIDA:** Ao implementar o `HttpModule` real (T-405), a leitura direta do código nativo da lib (`android/.../Server.java`, `HttpServerModule.java`, conferido nas versões 1.2.9 e 1.3.2, a mais recente) mostrou que a premissa central da Seção 2.1 ("o módulo nativo processa o corpo do request chunk-by-chunk; a camada JS recebe eventos de chunk") **é falsa para esta lib**: `Server.serve()` usa `session.parseBody(files)` do NanoHTTPD, que bufferiza o corpo inteiro nativamente antes de emitir **um único evento JS** com o corpo completo em `postData`; `respond(requestId, code, type, body)` envia a resposta inteira como **uma string única** via `newFixedLengthResponse` — sem streaming de entrada nem de saída, em nenhuma das duas versões testadas. Isso não passa no critério decisivo definido pela própria T-202 ("upload multipart ≥ 1 GB com streaming") nem no "nunca bufferizar corpo inteiro" da T-403. A ANOTAÇÃO DO REVISOR (§4.1) — que já sinalizava esse risco e recomendava inverter a ordem — se confirma. **Decisão revertida**: adota-se a Alternativa 2.2 (`react-native-tcp-socket` + parser HTTP/1.1 próprio, simplificado — sem `Transfer-Encoding: chunked`, sem keep-alive), conforme o plano de contingência da Seção 6.1-B, agora promovida a implementação principal. `react-native-http-bridge-refurbished` é removida das dependências. Status muda de "Accepted" para **"Superseded"** (por esta emenda).
