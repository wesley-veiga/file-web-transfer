 ...
 *   que retorna um array de URIs extraído de `EXTRA_STREAM` (ACTION_SEND) ou
 *   `EXTRA_STREAM_URIS` (ACTION_SEND_MULTIPLE) do Intent inicial da MainActivity.
 ...
 * - ACTION_SEND_MULTIPLE: array de Uri via EXTRA_STREAM_URIS
 *
...
class ${MODULE_CLASS_NAME}(reactContext: ReactApplicationContext) :
     ...
     * - ACTION_SEND_MULTIPLE: retorna array com N URIs (EXTRA_STREAM_URIS)
        ...
        try {
            ...
            when {
                ...
                Intent.ACTION_SEND_MULTIPLE == intent?.action -> {
                    ...
                    val uriList = intent.getParcelableArrayListExtra<Uri>(Intent.EXTRA_STREAM_URIS)
                    if (uriList != null) {
                        uriList.forEach { uri ->
                            if (uri != null) {
                                uris.add(uri.toString())
            ...
            promise.resolve(uris.toTypedArray())
        } catch (e: Exception) {
...
function withShareIntentPayloadNativeFiles(config) {
  return withDangerousMod(config, [
    ...
    async (config) => {
      ...
      if (!packageName) {
        throw new Error(
          'withShareIntentPayload: config.android.package não definido em app.json',
        );
      }
