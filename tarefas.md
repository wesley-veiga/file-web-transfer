...
**Revisão 1.15 (2026-09-15):** nova **T-915** — a pedido do usuário, que reportou a caixa "Pasta Configurada" mostrando a URI SAF crua (percent-encoding e o volume id técnico `primary`) em vez de algo legível — nova função `humanizeFolderUri` traduz isso para rótulos como "Armazenamento interno / Documents". Ver T-915 para detalhes.
...
- [x] **T-915 · Exibir pasta configurada com nome legível, não a URI SAF crua** ⬅ T-802
  Reportado pelo usuário: a caixa "Pasta Configurada" mostrava a URI SAF bruta (ex.: `content://com.android.externalstorage.documents/tree/primary%3ADocuments`), com percent-encoding e o volume id técnico `primary` — nada disso significa algo para quem não conhece a implementação do Android. Nova função `humanizeFolderUri` (`folderSharingService.ts`) decodifica o document id da URI de árvore SAF e traduz `primary` (armazenamento interno do aparelho) para "Armazenamento interno" e qualquer outro volume (cartão SD, pen drive OTG — id opaco tipo `1234-5678`) para "Armazenamento externo", seguido do caminho decodificado e legível (ex.: "Armazenamento interno / Download / Teste"). URIs de outros provedores SAF (Google Drive, Dropbox etc., também selecionáveis no seletor de pasta do SO) ou em formato inesperado caem num fallback seguro em vez de arriscar um rótulo incorreto.
  *Pronto quando:* a caixa "Pasta Configurada" nunca mostra percent-encoding nem o volume id técnico; validado visualmente em emulador (`Armazenamento interno / Documents` em vez da URI crua); suíte de testes cobrindo armazenamento interno, externo, raiz, caracteres especiais e formatos inesperados.
